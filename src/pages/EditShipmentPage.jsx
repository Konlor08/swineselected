...
const OFFLINE_BANNER_TEXT =
  "ขณะนี้อุปกรณ์ออฟไลน์ ระบบจะยังไม่สามารถโหลดข้อมูลฟาร์ม, flock และค้นหาเบอร์หมูใหม่จากเซิร์ฟเวอร์ได้";
...
function normalizeDraftMatches(rows) {
  if (!Array.isArray(rows)) return [];
  return rows
    .map((m) => ({
      shipment_id: clean(m?.shipment_id),
      shipment_no: clean(m?.shipment_no),
      selected_date: clean(m?.selected_date),
      created_at: clean(m?.created_at),
    }))
    .filter((m) => m.shipment_id);
}

function normalizeSwineSearchResults(rows) {
  if (!Array.isArray(rows)) return [];
  return rows
    .map((row) => ({
      key: clean(row?.key),
      ...
      draft_matches: normalizeDraftMatches(row?.draft_matches),
      draft_match_count: Number(row?.draft_match_count || 0),
    }))
    .filter((row) => row.key && row.swine_code);
}

function normalizeShipmentHeader(header) {
  if (!header) return null;
  return {
    id: clean(header?.id),
    shipment_no: clean(header?.shipment_no),
    selected_date: clean(header?.selected_date),
    delivery_date: clean(header?.delivery_date),
    from_farm_code: clean(header?.from_farm_code),
    from_farm_name: clean(header?.from_farm_name),
    from_flock: clean(header?.from_flock),
    to_farm_id: clean(header?.to_farm_id),
    remark: clean(header?.remark),
    status: clean(header?.status),
    created_at: clean(header?.created_at),
    updated_at: clean(header?.updated_at),
    to_farm: header?.to_farm
      ? {
          id: clean(header?.to_farm?.id),
          farm_code: clean(header?.to_farm?.farm_code),
          farm_name: clean(header?.to_farm?.farm_name),
        }
      : null,
  };
}
...
function normalizeDraftItem(item) {
  ...
  return {
    ...
    house_no: clean(item?.house_no),
    flock: clean(item?.flock),
    birth_date: clean(item?.birth_date),
    teats_left: clean(item?.teats_left),
    teats_right: clean(item?.teats_right),
    backfat: clean(item?.backfat),
    weight: clean(item?.weight),
  };
}

function normalizeDraftOriginalSnapshot(snapshot) {
  if (!snapshot) return null;
  return {
    teats_left: clean(snapshot?.teats_left),
    teats_right: clean(snapshot?.teats_right),
    backfat: clean(snapshot?.backfat),
    weight: clean(snapshot?.weight),
    remark: clean(snapshot?.remark),
    to_farm_id: clean(snapshot?.to_farm_id),
    delivery_date: clean(snapshot?.delivery_date),
  };
}

function hasAnyLocalDraftContent({
  selectedFarmCode,
  selectedFlock,
  swineSearchQ,
  swineSearchResults,
  selectedSwineResultKey,
  selectedShipmentId,
  selectedDraftItem,
  editRemark,
  editToFarmId,
  ...
  return Boolean(
    clean(selectedFarmCode) ||
      clean(selectedFlock) ||
      clean(swineSearchQ) ||
      clean(selectedSwineResultKey) ||
      clean(selectedShipmentId) ||
      selectedDraftItem ||
      clean(editRemark) ||
      clean(editToFarmId) ||
      clean(editDeliveryDate) ||
      clean(createToFarmId) ||
      clean(createTeatsLeft) ||
      clean(createTeatsRight) ||
      clean(createBackfat) ||
      clean(createWeight) ||
      (Array.isArray(swineSearchResults) && swineSearchResults.length > 0)
  );
}
...
export default function EditShipmentPage() {
  ...
  const draftKey = useMemo(() => getLocalDraftKey(userId), [userId]);
  ...
  const handleBack = useCallback(() => {
    const hasLocalContent = hasAnyLocalDraftContent({
      selectedFarmCode,
      selectedFlock,
      swineSearchQ,
      swineSearchResults,
    ...
    if (hasLocalContent) {
      const ok = window.confirm(
        "ยืนยัน Back ใช่หรือไม่?\nระบบจะล้างข้อมูลค้างในหน้านี้แล้วกลับหน้าก่อนหน้า"
      );
      if (!ok) return;
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      setFilterDateFrom(clean(parsed?.filterDateFrom) || initialDateFrom);
      setFilterDateTo(clean(parsed?.filterDateTo) || initialDateTo);
      setSelectedFarmCode(clean(parsed?.selectedFarmCode));
      setSelectedFlock(clean(parsed?.selectedFlock));
      setSwineSearchQ(clean(parsed?.swineSearchQ));

      setSwineSearchResults(normalizeSwineSearchResults(parsed?.swineSearchResults));
      setSelectedSwineResultKey(clean(parsed?.selectedSwineResultKey));
      setSwineSearchMode(clean(parsed?.swineSearchMode) || "idle");

      setSelectedShipmentId(clean(parsed?.selectedShipmentId));
      setShipmentHeader(normalizeShipmentHeader(parsed?.shipmentHeader));
      setSelectedDraftItem(normalizeDraftItem(parsed?.selectedDraftItem));
      setDraftOriginalSnapshot(
        normalizeDraftOriginalSnapshot(parsed?.draftOriginalSnapshot)
      );

      setEditRemark(clean(parsed?.editRemark));
      setEditToFarmId(clean(parsed?.editToFarmId));
      setEditToFarmMeta(parsed?.editToFarmMeta || null);
      setEditDeliveryDate(clean(parsed?.editDeliveryDate));

      setCreateToFarmId(clean(parsed?.createToFarmId));
      setCreateTeatsLeft(clean(parsed?.createTeatsLeft));
      setCreateTeatsRight(clean(parsed?.createTeatsRight));
      setCreateBackfat(clean(parsed?.createBackfat));
      setCreateWeight(clean(parsed?.createWeight));
      ...
      console.error("restore edit local draft error:", e);
  ...
  useEffect(() => {
    ...
    const payload = {
      ...
      selectedFlock,
      swineSearchQ,
      swineSearchResults: normalizeSwineSearchResults(swineSearchResults),
      selectedSwineResultKey,
      swineSearchMode,
      selectedShipmentId,
      shipmentHeader: normalizeShipmentHeader(shipmentHeader),
      selectedDraftItem: normalizeDraftItem(selectedDraftItem),
      draftOriginalSnapshot: normalizeDraftOriginalSnapshot(draftOriginalSnapshot),
      editRemark,
      editToFarmId,
      editToFarmMeta,
      editDeliveryDate,
    ...
    const hasContent = hasAnyLocalDraftContent({
      selectedFarmCode,
      selectedFlock,
      swineSearchQ,
      swineSearchResults,
    ...
    try {
      if (!hasContent) {
        ...
        if (draftNotice && draftNotice.includes("local")) {
          setDraftNotice("");
        }
        return;
      }
      ...
      if (nearLimit) {
        setDraftNotice(
          `พื้นที่ local draft ใกล้เต็มแล้ว (${formatBytes(
            bytes
          )}) กรุณาบันทึกข้อมูลลงระบบโดยเร็ว`
        );
      } else if (draftNotice && draftNotice.includes("local draft")) {
        setDraftNotice("");
      }
    } catch (e) {
      console.error("save edit local draft error:", e);
      if (isQuotaExceededError(e)) {
        ...
        setDraftNotice(
          "พื้นที่ localStorage ไม่พอแล้ว ระบบอาจเก็บงานค้างเพิ่มไม่ได้ กรุณาบันทึกข้อมูลลงระบบโดยเร็ว"
        );
      } else {
  ...
  }, [farmOptions, initialFarmCode, selectedFarmCode, isOffline]);
  ...
  }, [selectedFarmCode, flockOptions, initialFlock, selectedFlock, isOffline]);
    ...
    async (queryText) => {
      ...
      try {
        ...
        if (shipmentIds.length > 0) {
          ...
          draftResults = Array.from(resultMap.values())
            .map((row) => ({
              ...row,
        ...
        setSwineSearchResults(mergedResults);
        setSelectedSwineResultKey((prev) => {
          const exists = mergedResults.some(
        ...
        if (!mergedResults.length) {
          ...
          mergedResults.some((x) => x.source_type === "draft") &&
          mergedResults.some((x) => x.source_type === "available")
        ) {
          setSwineSearchMode("mixed");
        } else if (mergedResults.some((x) => x.source_type === "draft")) {
  ...
  async function handleDeleteDraftSelectedSwine() {
    if (isOffline) {
      setMsg(
        "ลบไม่สำเร็จ: เชื่อมต่อ server ไม่ได้ กรุณาตรวจสอบอินเทอร์เน็ตแล้วลองใหม่"
      );
      return;
    }
  ...
  return (
    <div className="page" style={{ overflowX: "hidden" }}>
      ...
      >
        {isOffline ? (
          <div className="card" style={{ padding: 12, ...cardStyle }}>
            ...
            >
              {OFFLINE_BANNER_TEXT}
              <br />
              ข้อมูลที่เปิดค้างไว้ยังอยู่ในเครื่อง แต่จะค้นหา/โหลดข้อมูลใหม่และบันทึกลงระบบไม่ได้จนกว่าจะต่ออินเทอร์เน็ต
            </div>
          </div>
        ) : null}
        ...
        <div className="card" style={{ padding: 12, ...cardStyle }}>
          <div style={{ fontWeight: 800, marginBottom: 8 }}>สถานะ local draft</div>
          <div className="small" style={{ color: "#374151", lineHeight: 1.8 }}>
            ขนาด local draft: <b>{formatBytes(draftInfo.bytes)}</b>
            <br />
            {draftInfo.lastSavedAt ? (
              <>
                บันทึกล่าสุดในเครื่อง:{" "}
                <b>{new Date(draftInfo.lastSavedAt).toLocaleString()}</b>
              </>
            ) : (
              <>ยังไม่มี local draft ในเครื่อง</>
            )}
            <br />
            จำนวนผลค้นหาที่เก็บไว้: <b>{swineSearchResults.length}</b>
          </div>
        </div>
                      ...
                      >
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div
                            ...
                            style={{
                              marginTop: 4,
                              color: row.source_type === "draft" ? "#374151" : "#166534",
                              fontWeight: 700,
                            }}
                          >
                            {row.source_type === "draft"
                              ? `status: ${row.status || "draft"} | อยู่ใน draft ${row.draft_match_count} รายการ`
                              : `status: ${row.status || "available"} | คัดได้ในหน้านี้`}
                          </div>
                        </div>
                      </div>
                    </button>
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                      <button
                        ...
                        disabled={savingDraftItem || deletingDraftItem || isOffline}
                      >
