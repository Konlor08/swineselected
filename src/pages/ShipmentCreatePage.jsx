...
function parseShipmentConflictMessage(message) {
  const text = clean(message);
  if (!text) return null;

  let m = text.match(/^เบอร์\s+(\d+)\s+อยู่ใน shipment สถานะ\s+(.+?)\s+แล้ว$/);
  if (m) {
    return {
      swineCode: clean(m[1]),
      nextMessage: `ลบเบอร์ ${clean(m[1])} ออกจาก list แล้ว เพราะอยู่ใน shipment สถานะ ${clean(m[2]) || "-"}`,
    };
  }

  m = text.match(/^เบอร์\s+(\d+)\s+อยู่สถานะ\s+(.+?)\s+แล้ว$/);
  if (m) {
    return {
      swineCode: clean(m[1]),
      nextMessage: `ลบเบอร์ ${clean(m[1])} ออกจาก list แล้ว เพราะอยู่สถานะ ${clean(m[2]) || "-"}`,
    };
  }
...
export default function ShipmentCreatePage() {
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    try {
      ...
      const conflictInfo = parseShipmentConflictMessage(e?.message);
      if (conflictInfo?.swineCode) {
        setPickedRows((prev) =>
          prev.filter((row) => clean(row.swine_code) !== conflictInfo.swineCode)
        );
        setMsg(conflictInfo.nextMessage);
      } else {
