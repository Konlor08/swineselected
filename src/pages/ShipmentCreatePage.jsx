...
function parseSwineConflictErrorMessage(message) {
  const text = clean(message);
  if (!text) return null;

  const shipmentMatch = text.match(/^เบอร์\s+(\d{10})\s+อยู่ใน\s+shipment\s+สถานะ\s+(.+?)\s+แล้ว$/);
  if (shipmentMatch) {
    return {
      swineCode: clean(shipmentMatch[1]),
      type: "shipment",
      status: clean(shipmentMatch[2]),
    };
  }

  const stateMatch = text.match(/^เบอร์\s+(\d{10})\s+อยู่สถานะ\s+(.+?)\s+แล้ว$/);
  if (stateMatch) {
    return {
      swineCode: clean(stateMatch[1]),
      type: "state",
      status: clean(stateMatch[2]),
    };
  }
...
export default function ShipmentCreatePage() {
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    try {
      ...
      const swineConflict = parseSwineConflictErrorMessage(e?.message);

      if (swineConflict?.swineCode) {
        setPickedRows((prev) =>
          prev.filter((row) => clean(row.swine_code) !== swineConflict.swineCode)
        );

        setMsg(
          swineConflict.type === "shipment"
            ? `ลบเบอร์ ${swineConflict.swineCode} ออกจาก list แล้ว เพราะอยู่ใน shipment สถานะ ${
                swineConflict.status || "-"
              }`
            : `ลบเบอร์ ${swineConflict.swineCode} ออกจาก list แล้ว เพราะอยู่สถานะ ${
                swineConflict.status || "-"
              }`
        );
      } else {
